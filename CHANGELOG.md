...
**Branch:** `serverless`
**上游基准：** `upstream/main` @ `26f6d41`（上次同步：`19bfce3`）

### 合并的上游变更

从上游合并了 26 个新提交（`19bfce3..26f6d41`），包含 7 个 PR 的合并：

#### 1. PR [#346](https://github.com/chenyme/grok2api/pull/346) — 修复 get_tokens 端点缺失 get_config 导入

- **文件：** `app/api/v1/admin/token.py` — 在 `get_tokens()` 函数内添加 `from app.core.config import get_config` 局部导入，修复 PR #315 遗漏的导入导致的 `NameError`

#### 2. PR [#344](https://github.com/chenyme/grok2api/pull/344) — curl_cffi 导入修复
...
#### 3. PR [#315](https://github.com/chenyme/grok2api/pull/315) — Token consumed mode 配额管理
...
- **~~上游 Bug~~（已由上游 PR [#346](https://github.com/chenyme/grok2api/pull/346) 修复）：** `app/api/v1/admin/token.py` 调用 `get_config()` 但遗漏导入，导致 `/v1/admin/tokens` 端点抛出 `NameError` → HTTP 500。上游在函数内部添加局部导入修复，本 fork 已同步采用上游方案

#### 4. PR [#314](https://github.com/chenyme/grok2api/pull/314) — Serverless 冷启动数据库初始化修复
...
#### 5. PR [#312](https://github.com/chenyme/grok2api/pull/312) — Token 刷新逻辑与过期处理修复
...
#### 6. PR [#311](https://github.com/chenyme/grok2api/pull/311) — IP 轮换代理池支持
...
#### 7. PR [#295](https://github.com/chenyme/grok2api/pull/295) — 视频公共资源链接生成
